## KOSHIAN 自動リンク生成 改
このアドオンは[Pachira](https://addons.mozilla.org/ja/firefox/user/anonymous-a0bba9187b568f98732d22d51c5955a6/)氏の[KOSHIAN 自動リンク生成](https://addons.mozilla.org/ja/firefox/addon/koshian-autolink-futaba/)アドオンを改変したものです  
「引用された塩のファイル名のリンクを作成する」オプションをオリジナル版に追加しています  
（デフォルト：無効）  
これはKOSHIAN 自動リンク生成アドオンが引用された塩のファイル名をリンク化してしまい、  
[KOSHIAN 引用をポップアップで表示](https://addons.mozilla.org/ja/firefox/addon/koshian-popup-quote/)アドオンで認識できないため、  
オプションでリンク化するか選択できるようにしたものです  
オプションを有効にすればオリジナル版と同じ動作になります  

## インストール
[GitHub](https://github.com/akoya-tomo/koshian_autolink_futaba_kai/releases/download/v1.0.0/koshian_autolink_futaba_kai-1.0.0-an.fx.xpi)

## 更新履歴

* v1.0.0 2017-12-18
  - KOSHIAN 自動リンク生成 v1.4.3ベース
  - 「引用された塩のファイル名のリンクを作成する」オプションを追加
